...
class BreathingLines implements AudioReactiveAnimationInterface {
  ...
  public void displayFrame(FFT fft) {
    ...
    for (int i = 0; i < numLines; i++) {
      ...
      float wave = 20 * sin((HALF_PI)* sin((-frameCount + waveOffset) / 40.0f)) + 20;
...
class Grid implements AudioReactiveAnimationInterface{
  ...
  public void displayFrame(FFT fft){
    ...
    stroke(255);
    colorMode(HSB);
    strokeWeight(0.3f+3*fft.getBand(80) );
...
class SineWaveShader extends AbstractShaderAnimation {
  ...
  protected void setAdditionalParams(FFT fft) {
    shader.set("colorMult", constrain(fft.getBand(100), 0.5f, 5), constrain(fft.getBand(300), 0.5f, 2.0f)); // 10 -> 100
  ...
  public void settings() {  size(980, 320, P3D); }
